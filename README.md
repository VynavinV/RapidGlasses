...
1. **Stage 1 — passive/quick primary screen.** Pupil diameter + variance (glasses IR eye camera), blink rate/duration, an optional pupillary light response (PLR) test, and heart rate / HRV from **"Presage"** (referenced as `hrv.source: "Presage (laptop)"`). "Presage" is Presage Technologies' **SmartSpectra SDK** — camera-based rPPG vitals measurement — and it is now **integrated and working** via `vitals-server/` (§4.7): a Node.js bridge process (Presage ships no Python SDK) that receives webcam frames from `tracking.py` and produces live pulse rate, breathing rate, HRV, and an arterial pressure trace. Produces a `stage1_composite_score` (the composite scoring itself is still unbuilt).
...
├── vitals-server/            Node.js Presage SmartSpectra bridge (heart rate / vitals) — spawned by secondcheck.py, see §4.7
│   ├── server.js               The whole server: POST /frame ingest → SDK, GET /vitals JSON, port 3002
│   ├── package.json            Single dependency: @smartspectra/node-sdk (koffi-based FFI binding)
│   └── node_modules/           npm-installed, gitignored territory — includes per-platform native runtime (~hundreds of MB)
...
There is **no build tooling, no test suite, no CI config, no database, no persistence layer** anywhere in this repo. The only `package.json` is `vitals-server/package.json` (one dependency, no scripts, no bundler — plain `node server.js`). Nothing here writes assessment results anywhere permanent yet (see §7, Gaps).
...
└─────────────────────────────┘         │  ┌────────────────────────────────┐  │
                                          ...
                                          │  │   GET  /tracking/video (MJPEG)   │  │
                                          │  └──────────────┬─────────────────┘  │
                                          └─────────────────┼────────────────────┘
                                                             │ opens (sole camera owner)
                                                             ▼
                                          ┌──────────────────────────────────────┐
                                          ...
                                          └──────────────────────────────────────┘
                                                            │ forwards every raw RGB frame
                                                            │ (HTTP POST, localhost only)
                                                            ▼
                                          ┌──────────────────────────────────────────┐
                                          │  vitals-server/server.js (Node, :3002)   │
                                          │  spawned + killed by secondcheck.py      │
                                          │   POST /frame  ← frames from tracking.py │
                                          │   GET  /vitals → pulse/breathing JSON    │
                                          │  Presage SmartSpectra SDK (headless      │
                                          │  custom-input mode) → Presage cloud      │
                                          │  auth via SMARTSPECTRA_API_KEY (.env)    │
                                          └──────────────────────────────────────────┘
...
**Key architectural fact #3:** the assessment runtime is now **two processes acting as one**: `secondcheck.py` spawns `vitals-server/server.js` (Node, port 3002) at startup and terminates it at exit (`atexit`). This exists because Presage's SmartSpectra SDK ships for Node (and other platforms) but **not Python**, and `main.py` runs on QNX where Node-vs-Python compatibility juggling wasn't worth it — so the Python side keeps camera ownership and simply forwards frames over localhost HTTP. **One camera, one owner:** `tracking.py`'s loop is the only code that opens the webcam; the webpage's overlay (MJPEG re-stream) and Presage (raw-frame forwarding) are both fed from that single capture loop, deliberately avoiding the "two processes fighting over the webcam" failure mode.
...
| 7 | `vor` | Vestibulo-ocular reflex test: dot pulses in place, patient turns head side-to-side while fixating, 20s | Auto after fixed duration; tracking deliberately **keeps running** (Presage needs continuous frames) |
...
**Always-on monitor overlays (not part of the screen state machine):** two fixed-position elements sit on top of every screen, from page load to page close:

- **Camera monitor** (`#cam-overlay`, bottom-right, FaceTime-style): a plain `<img>` whose `src` is the `/tracking/video` MJPEG stream (§4.3) — the **same annotated view** the old `TRACKING_DEBUG=1` OpenCV window showed (TRACKING/NO FACE header, baseline crosshair, gain-magnified sway arrow, fps, scrolling sway trace), so the tester can always see exactly what the tracker sees. `width: 320px; height: auto` — do **not** reintroduce a fixed-aspect box with `object-fit: cover`; that crops the annotation header/trace bars off (a bug that was hit and fixed). On stream error it retries every 2s with a cache-busting query param.
- **Heart-rate badge** (`#hr-badge`, top-right) **+ status line** (`#hr-status`, just below it): polls `GET http://localhost:3002/vitals` (the Node Presage bridge, §4.7) every 1s. Shows live bpm (heart pulses via CSS animation when a reading is fresh, i.e. `bpm != null` and `updated` < 10s old, else `--`). The status line always shows *why* a reading is missing — the SDK's live validation hint (e.g. "Place more of the chest in view.") if present, else `waiting for pulse — <status>` — plus the live breathing rate (`N br/min`) so it's visible that data is flowing. `vitals server offline` when the fetch fails entirely.

**Camera lifecycle (changed):** the camera now starts at **page load** (`trackStart()` + `connectCam()` run at script top level), not at calibration, and is **never stopped by the UI** — Presage heart-rate measurement needs continuous frames for the whole session. Each test's `trackStart()` call only rebaselines the sway running-mean (the thread/camera persist). The old `trackStop()` calls at end-of-VOR and in `resetAll()` were removed (`resetAll()` now calls `trackStart()` to rebaseline instead); the `trackStop()` helper function remains defined for manual/QNX use but is unused by the flow. The `calibration` screen still works unchanged — it just finds the camera already warm.

**Network calls made by this file:**
| Call | Target | When |
|---|---|---|
| `POST /tracking/start` | `http://localhost:3001` | Page load (starts the camera), then entering calibration, balance, pursuit, VOR, and `resetAll()` (each just rebaselines the mean) |
| `GET /tracking/video` | `http://localhost:3001` | Continuously — MJPEG `<img>` stream for the camera overlay, from page load |
| `GET /tracking/snapshot` | `http://localhost:3001` | Polled during calibration only, at `CALIB_POLL_MS` |
| `GET /vitals` | `http://localhost:3002` (Node bridge) | Every 1s, from page load — heart-rate badge + status line |
...
**Vitals-server child process:** on `__main__` startup, `_start_vitals_server()` spawns `node server.js` with `cwd=vitals-server/` via `subprocess.Popen` and registers an `atexit` hook that terminates it if still alive. The child inherits the parent's environment, which is how `SMARTSPECTRA_API_KEY` from the project `.env` (loaded by `load_dotenv()` at the top of this file) reaches the Node process. If `node` isn't installed (`OSError`), it prints `vitals server not started (...) — heart rate unavailable` and continues — the assessment runs fine without vitals, matching the repo-wide "backend hiccups never block the assessment" pattern. `app.run(port=3001, threaded=True)` — `threaded=True` matters now because the MJPEG `/tracking/video` stream holds a request thread open per viewer.
...
**The loop now does three jobs per frame** (added when Presage was integrated — one camera, one owner, three consumers):
1. **Sway tracking** — unchanged: nose-tip landmark, running mean, deviation into `_state`.
2. **Annotated MJPEG publishing** — `_publish()` runs `_draw_debug()` on **every** frame (it used to run only under `TRACKING_DEBUG=1`), JPEG-encodes the annotated frame (quality 70), and stores the bytes in module-level `_jpeg` under `_lock`. `/tracking/video` serves this to the webpage's camera overlay; the optional local `cv2.imshow` debug window now just displays the same annotated frame.
3. **Raw-frame forwarding to Presage** — `_forward_frame()` POSTs each frame's raw RGB bytes (the `rgb` array already computed for MediaPipe, `.tobytes()`) to the Node bridge at `VITALS_FRAME_URL` with headers `X-Width` / `X-Height` / `X-Ts` (`time.monotonic()` in µs — **monotonic on purpose**; the SmartSpectra SDK rejects wall-clock/non-monotonic timestamps with `kNonMonotonicTimestamp`). Uses a persistent `requests.Session`, `timeout=0.5`; on any `RequestException` it backs off 2s before retrying, so a missing/dead Node process costs the loop nothing. A failure here must never stall sway tracking — preserve that if refactoring.
...
SWAY_GAIN        = 12         # visual-only magnification factor for the annotation overlay
DEV_HISTORY      = 240        # sway-trace history length (drawn on every published frame)
VITALS_FRAME_URL = env VITALS_FRAME_URL, default http://localhost:3002/frame  # Node Presage bridge
```
...
| `POST` | `/tracking/stop` | Signals the thread to stop, joins with 3s timeout, releases camera | Returns `{status: "stopped"}`. **No longer called by `index.html`'s flow** (camera stays on for Presage) — kept for QNX/manual use |
| `GET` | `/tracking/snapshot` | Pure read of current shared state, no camera work on this thread | See response shape below |
| `GET` | `/tracking/video` | MJPEG stream (`multipart/x-mixed-replace`) of the annotated camera view | Generator yields the latest `_jpeg` at ~20fps (`sleep(0.05)`); same drawing as the debug window. Holds one Flask request thread per connected viewer (hence `threaded=True` in §4.2). Yields nothing (idles) until the loop publishes its first frame; `_jpeg` is cleared to `None` on loop shutdown |
...
**Annotation visualization:** `_draw_debug()` draws the tracked point, the running-mean baseline crosshair, a gain-magnified sway arrow, a live deviation circle, fps/frame-count readout, and a scrolling sway-trace graph. This is **no longer debug-only** — it runs on every frame and is what `/tracking/video` streams to the webpage's camera overlay. `TRACKING_DEBUG=1` additionally mirrors the same annotated frame into a local `cv2.imshow` window (must run inside the tracking loop's own thread since only one process can hold the camera).
...
### 4.7 `vitals-server/` — the Presage SmartSpectra bridge (Node.js, port 3002)

**Role:** A small Node.js process (`server.js`, plain `http` module, no Express/framework) that wraps Presage Technologies' **SmartSpectra SDK** (`@smartspectra/node-sdk` — camera-based rPPG vitals: pulse, breathing, HRV) and bridges it to the rest of the (Python) system. This exists because **Presage ships no Python SDK**; the Node SDK is a koffi-based FFI binding over their native C ABI runtime (no node-gyp/native build step — `npm install` pulls per-platform prebuilt runtime packages, a few hundred MB, which is expected). This is the Stage 1 `hrv.source: "Presage (laptop)"` integration from `report_data.json` (§4.10), now real.

**Lifecycle:** never run by hand — `secondcheck.py` spawns it at startup and terminates it at exit (§4.2). On `SIGTERM` it awaits `sdk.destroy()` before exiting. It is deliberately **failure-tolerant at every layer**: no API key → runs anyway with `status: "no api key (set SMARTSPECTRA_API_KEY)"`; `node_modules` missing → runs anyway with `status: "sdk unavailable: ..."`; in both cases `/frame` still 204s and `/vitals` still answers, so the webpage's badge shows `--` with the reason instead of erroring.

**SDK wiring (headless custom-input mode):** `sdk.useCustomInput(FrameTransform.kNone)` + `sdk.start()` — the SDK does **not** open a camera; `tracking.py` owns the camera and pushes frames in (§4.3, one-camera-one-owner). Requested metrics: `[...cardioMetrics, ...breathingMetrics]` (MetricType codes; cardio = `PULSE_RATE`=15, `ARTERIAL_PRESSURE_TRACE`=16, `HRV`=17 — cardio fields are empty unless explicitly requested, the default bundle is breathing-only).

**Endpoints:**

| Method | Path | Purpose | Detail |
|---|---|---|---|
| `POST` | `/frame` | Frame ingest from `tracking.py` | Body = raw RGB bytes; headers `X-Width`/`X-Height`/`X-Ts` (µs, **monotonic** — wall-clock timestamps get rejected by the SDK as `kNonMonotonicTimestamp`). Validates `buf.length === w*h*3`, then `sdk.sendFrame(buf, w, h, w*3, PixelFormat.kRGB, BigInt(ts))`. Always `204`, even with no SDK |
| `GET` | `/vitals` | Live vitals JSON for the webpage | `Access-Control-Allow-Origin: *` so the `file://` page can poll it. Shape below |

**`/vitals` response shape:**
```json
{
  "bpm": 83,                  // latest pulse, rounded; null until first lock
  "bpmConfidence": 84.6,      // SDK confidence for that reading
  "breathing": 19,            // breaths/min, rounded
  "status": "running",        // SDK lifecycle: uninitialized|idle|starting|running|stopping|error, or a startup-failure reason
  "hint": null,               // latest SDK validation hint when not measurement-ready, e.g. "Place more of the chest in view." — null when OK
  "lastMetrics": "cardio(arterialPressureTrace,hrv,pulseRate)",  // debug: shape of the latest metrics payload
  "updated": 1784408651234    // Date.now() of the last bpm update — the page treats bpm >10s old as stale
}
```

**Metrics payload — the field-path trap (learned the hard way):** the decoded payload puts heart rate at **`metrics.cardio.pulseRate[].value`** (with `.confidence`, `.stable`, `.timestamp`), *not* at `pulse.rate` — even though the SDK's bundled protobuf typings also define a `Pulse { rate }` message that the payload never populates. Breathing is at `metrics.breathing.rate[].value` (plus `upperTrace`/`lowerTrace` chest/abdomen traces, which arrive first and sometimes carry entries with no `value`). HRV lives at `metrics.cardio.hrv[]` (`rmssd`, `meanNn`, `sdnn`, `baevsky`) but reads `NaN` until enough beats accumulate (~a minute). To guard against this class of bug, the server logs each **distinct payload shape** once (`[vitals] metrics shape: ... | sample: ...`) and mirrors the latest shape into `/vitals.lastMetrics`.

**Measurement conditions (observed):** pulse locks within a few seconds of a well-lit, reasonably close, still face at ~30fps; breathing wants the **upper chest in frame** too (hint: "Place more of the chest in view."). Validation hints come from the SDK's `validationStatus` event (`kNoFaceFound`, `kTooDark`, `kFaceSizeOutOfRange`, `kChestNotVisible`, `kFrameRateTooLow`, …) and are surfaced verbatim to the page's status line. Confidence in the 50–90 range has been normal in practice.

**Config:** `SMARTSPECTRA_API_KEY` (required for real measurements — free key from <https://physiology.presagetech.com>, lives in the project `.env`, reaches Node via env inheritance from `secondcheck.py`), `VITALS_PORT` (default 3002). Setup steps are in SETUP.md §5b.
...
| `POST` | `/tracking/stop` | tracking.py (Blueprint) | none | Stop tracking thread, release webcam (no longer called by the UI flow — QNX/manual use) |
| `GET` | `/tracking/snapshot` | tracking.py (Blueprint) | none | Poll current sway/face-detection state |
| `GET` | `/tracking/video` | tracking.py (Blueprint) | none | Annotated MJPEG stream of the camera view — the webpage's bottom-right monitor overlay |

**Process 1b — the Presage vitals bridge, `localhost:3002`** (Node), spawned automatically by Process 1 (§4.7) — never launched by hand:

| Method | Path | Module | Auth | Purpose |
|---|---|---|---|---|
| `POST` | `/frame` | vitals-server/server.js | none | Raw RGB frame ingest from tracking.py's loop (headers `X-Width`/`X-Height`/`X-Ts` µs monotonic) |
| `GET` | `/vitals` | vitals-server/server.js | none (CORS `*`) | Live pulse/breathing/status/hint JSON, polled 1Hz by index.html's heart-rate badge |
...
| Presage SmartSpectra cloud (via `@smartspectra/node-sdk`) | vitals-server/server.js | `SMARTSPECTRA_API_KEY` env var (key from physiology.presagetech.com) | The SDK processes frames locally but authenticates/computes against Presage's service — vitals need network + a valid key. Metered/credit-based (`kCreditExhausted` is a real SDK error code). Failures degrade to `status`/`hint` fields in `/vitals`, never block the assessment. |
...
### 6.11 Vitals are displayed but not recorded
The Presage bridge (§4.7) produces live pulse/breathing/HRV, and the webpage shows them — but exactly like the sway data (§6.3), **nothing captures vitals into any per-session record**. The `stage1.hrv` block of `report_data.json` (`heart_rate_bpm`, `hrv_rmssd_ms`, `source: "Presage (laptop)"`) is the shape a real pipeline should fill from `/vitals` (or from richer extraction in `server.js` — HRV and the arterial pressure trace are already in the decoded payloads, just not exposed beyond `breathing`/`bpm`). Also note the browser-console side effect: before `secondcheck.py` is running, an open page logs a stream of `ERR_CONNECTION_REFUSED` for `:3002/vitals` — harmless, self-heals once the server starts.
...
| Live vitals via Presage (pulse, breathing, HRV) | **Built, functional** — Node bridge + frame forwarding + live webpage display; verified end-to-end with real readings. Displayed only, **not recorded** (§6.11) | `vitals-server/server.js`, `tracking.py`, `index.html` |
| Live camera monitor (annotated, in-page) | Built, functional — MJPEG overlay replaces the debug-window-only view | `tracking.py` (`/tracking/video`), `index.html` |
| Stage 1 capture pipeline (pupil/blink/PLR via glasses; HRV ✓) | **Mostly not started** — the HRV/heart-rate leg now exists (Presage, above); pupil/blink/PLR capture and Stage 1 scoring still have zero implementing code | `vitals-server/` (HRV leg only) |
...
| Vitals persistence into `report_data.json` (`stage1.hrv` block) | **Not started** — vitals are live-displayed but discarded, see §6.11 | — |
...
**The one-paragraph mental model:** three independent process groups exist (`secondcheck.py`:3001 running the Stage 3 assessment + head-sway tracking, which auto-spawns its Node sidecar `vitals-server/server.js`:3002 for Presage heart-rate; `main.py` a desktop-only pupil tracker with no server; `report.py`:8080 rendering a static sample report) plus a fully-specified-but-unbuilt data pipeline (Stage 1 capture → gate/fusion decision → `report_data.json`) that would connect them. Live vitals (pulse/breathing via Presage) now flow camera → tracking.py → Node bridge → webpage in real time, but like everything else are displayed and then discarded — nothing currently writes real data between any of these pieces. If you're asked to "unify" or "connect" this codebase, the work is almost certainly: (a) get Stage 3 to actually record its results instead of discarding them (§6.3), (b) shape those results into the `report_data.json` schema (§4.10, §6.8), (c) figure out what Stage 1 needs to be and build a capture flow for it (§6.4), and (d) write the fusion/gating logic that ties it all together and produces the final report payload.
...
- **The webcam has exactly one owner: `tracking.py`'s loop.** Everything else that needs webcam pixels — the webpage's annotated overlay (`/tracking/video` MJPEG) and Presage heart-rate (raw-frame POSTs to the Node bridge) — is fed *from* that loop. Never add a second `cv2.VideoCapture` or let the Presage SDK open the camera itself (it can; we deliberately use `useCustomInput()` instead) — that recreates the camera-conflict problem this design exists to prevent. See §4.3, §4.7.
- **Three server processes, three ports, no shared state:** `secondcheck.py` (Flask, 3001), `vitals-server/server.js` (Node, 3002 — auto-spawned by 3001, never run by hand), `report.py` (Flask, 8080). None of them import from each other or share a database — moving data between them today means writing new integration code, not flipping a flag.
- **Presage payload trap:** heart rate is at `metrics.cardio.pulseRate[].value` — NOT `pulse.rate`, even though the SDK's protobuf typings define a `Pulse` message that looks right and never gets populated. Cost a debugging round once already; `server.js` logs payload shapes to catch this class of bug. See §4.7.
...
- **"QNX" is a real external reference, not a placeholder to ignore.** `metadata.devices.fusion_node` in `report_data.json` says `"Raspberry Pi (QNX)"`, matching `SETUP.md`'s "what QNX talks to" endpoint table and `tracking.py`'s docstring ("QNX polls /tracking/snapshot"). No QNX-side code exists in this repo — integrating with it is an external system/hardware task, not something to stub out casually.
- **"Presage" is now identified and integrated:** it's Presage Technologies' SmartSpectra SDK (camera rPPG vitals), wrapped by `vitals-server/` (§4.7) because there is no Python SDK. `hrv.source: "Presage (laptop)"` in `report_data.json` refers to exactly this. Needs `SMARTSPECTRA_API_KEY` in `.env` and a one-time `npm install` in `vitals-server/` (SETUP.md §5b).
